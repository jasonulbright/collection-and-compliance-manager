# Releasing Collection and Compliance Manager
...
- `Module/CCMCommon.psd1`, `ModuleVersion`
- `start-ccm.ps1`, header line `Version    : <ver>`
...
```powershell
C:\projects\app-packager-suite\sync-suitecommon.ps1 -Consumer C:\projects\collection-and-compliance-manager -Check
```
...
git archive --format=zip -o CollectionAndComplianceManager-<ver>.zip v<ver> -- . ':(exclude)Tests' ':(exclude)*.Tests.ps1' ':(exclude)RELEASING.md'
sha256sum CollectionAndComplianceManager-<ver>.zip | sed 's/ \*/  /' > checksums.txt
```

Extract the zip to a temporary folder. Import `Module/CCMCommon.psd1` under Windows PowerShell 5.1. The import must succeed.
